...
int main() {
  ...
  for (int i = 8001; i <= 8003; i++) {
    xps_listener_create(epoll_fd, "0.0.0.0", i);
...
int xps_loop_create() { return epoll_create1(0); }
...
void xps_loop_detach(int epoll_fd, int fd) { epoll_ctl(epoll_fd, EPOLL_CTL_DEL, fd, NULL); }
...
void xps_loop_run(int epoll_fd) {
  while (1) {
    ...
    for (int i = 0; i < n_ready_fds; i++) {
      ...
      // Checking if fd is of a listener
      ...
      // Checking if fd is of a connection
